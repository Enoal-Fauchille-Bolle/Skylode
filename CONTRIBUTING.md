...
- **`skylode-core` on crates.io**, published by Trusted Publishing rather than a
  stored token, so no registry credential exists in this repository to leak. The job
  checks the registry first and skips if the version is already there — re-tagging is
